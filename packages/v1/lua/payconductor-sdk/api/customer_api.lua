...
function customer_api:delete_customers_by_id(id)
...
function customer_api:get_customers(page, page_size, period, email, name, end_date, start_date)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
...
function customer_api:get_customers_by_id(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
...
function customer_api:patch_customers_by_id(id, customer_1)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
...
function customer_api:post_customers(customer)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
