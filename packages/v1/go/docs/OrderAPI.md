...
------------- | ------------- | -------------
[**GetOrders**](OrderAPI.md#GetOrders) | **Get** /orders/ | List orders
[**GetOrdersById**](OrderAPI.md#GetOrdersById) | **Get** /orders/{id} | Get order by ID
[**PostOrders**](OrderAPI.md#PostOrders) | **Post** /orders/ | Create order
[**PostOrdersByIdRefund**](OrderAPI.md#PostOrdersByIdRefund) | **Post** /orders/{id}/refund | Refund order
...
func main() {
	...
	r, err := apiClient.OrderAPI.GetOrders(context.Background()).Period(period).Page(page).PageSize(pageSize).EndDate(endDate).StartDate(startDate).Id(id).Status(status).Execute()
	if err != nil {
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	r, err := apiClient.OrderAPI.GetOrdersById(context.Background(), id).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `OrderAPI.GetOrdersById``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.OrderAPI.PostOrders(context.Background()).PostOrdersRequest(postOrdersRequest).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `OrderAPI.PostOrders``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `PostOrders`: PostOrders200Response
	fmt.Fprintf(os.Stdout, "Response from `OrderAPI.PostOrders`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	r, err := apiClient.OrderAPI.PostOrdersByIdRefund(context.Background(), id).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `OrderAPI.PostOrdersByIdRefund``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
}
```
