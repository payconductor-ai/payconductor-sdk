...
class Cliente(BaseModel):
    ...
    """ # noqa: E501
    address: Optional[EndereODoCliente] = None
    document_number: Annotated[str, Field(strict=True)] = Field(description="CPF ou CNPJ do cliente sem formatação", alias="documentNumber")
    document_type: StrictStr = Field(alias="documentType")
    email: StrictStr = Field(description="Email do cliente")
    name: StrictStr = Field(description="Nome completo do cliente")
    phone_number: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Número de telefone do cliente no formato +55 DD 9XXXXXXXX", alias="phoneNumber")
    __properties: ClassVar[List[str]] = ["address", "documentNumber", "documentType", "email", "name", "phoneNumber"]
    ...
    def phone_number_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^\+(\d{1,2}) (\d{2}) (\d{8,9})$", value):
            raise ValueError(r"must validate the regular expression /^\+(\d{1,2}) (\d{2}) (\d{8,9})$/")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "address": EndereODoCliente.from_dict(obj["address"]) if obj.get("address") is not None else None,
