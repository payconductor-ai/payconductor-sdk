...
CARTODECRDITOCARD_ANY_OF_SCHEMAS = ["CartOTokenizado", "DadosCompletosDoCartO"]

class CartODeCrDitoCard(BaseModel):
    """
    CartODeCrDitoCard
    """

    # data type: CartOTokenizado
    anyof_schema_1_validator: Optional[CartOTokenizado] = None
    # data type: DadosCompletosDoCartO
    anyof_schema_2_validator: Optional[DadosCompletosDoCartO] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[CartOTokenizado, DadosCompletosDoCartO]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "CartOTokenizado", "DadosCompletosDoCartO" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = CartODeCrDitoCard.model_construct()
        error_messages = []
        # validate data type: CartOTokenizado
        if not isinstance(v, CartOTokenizado):
            error_messages.append(f"Error! Input type `{type(v)}` is not `CartOTokenizado`")
        else:
            return v

        # validate data type: DadosCompletosDoCartO
        if not isinstance(v, DadosCompletosDoCartO):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DadosCompletosDoCartO`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in CartODeCrDitoCard with anyOf schemas: CartOTokenizado, DadosCompletosDoCartO. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[CartOTokenizado] = None
        try:
            instance.actual_instance = CartOTokenizado.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[DadosCompletosDoCartO] = None
        try:
            instance.actual_instance = DadosCompletosDoCartO.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CartODeCrDitoCard with anyOf schemas: CartOTokenizado, DadosCompletosDoCartO. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], CartOTokenizado, DadosCompletosDoCartO]]:
