...
-- model import
local payconductor_sdk_post_withdraws_200_response = require "payconductor_sdk.model.post_withdraws_200_response"
local payconductor_sdk_post_withdraws_request = require "payconductor_sdk.model.post_withdraws_request"

local transfers_api = {}
local transfers_api_mt = {
	__name = "transfers_api";
	__index = transfers_api;
}

local function new_transfers_api(authority, basePath, schemes)
	local schemes_map = {}
...
function transfers_api:get_withdraws(page, page_size, end_date, start_date, id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/withdraws?endDate=%s&startDate=%s&page=%s&pageSize=%s&id=%s",
			self.basePath, http_util.encodeURIComponent(end_date), http_util.encodeURIComponent(start_date), http_util.encodeURIComponent(page), http_util.encodeURIComponent(page_size), http_util.encodeURIComponent(id));
	})
...
function transfers_api:get_withdraws_by_id(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
...
function transfers_api:post_withdraws(post_withdraws_request)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
	...
	if http_status:sub(1,1) == "2" then
		...
		return payconductor_sdk_post_withdraws_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
