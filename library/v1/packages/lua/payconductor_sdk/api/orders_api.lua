...
-- model import
local payconductor_sdk_post_orders_by_id_confirm_200_response = require "payconductor_sdk.model.post_orders_by_id_confirm_200_response"
local payconductor_sdk_post_orders_200_response = require "payconductor_sdk.model.post_orders_200_response"
local payconductor_sdk_post_orders_request = require "payconductor_sdk.model.post_orders_request"
local payconductor_sdk_post_orders_request_payment = require "payconductor_sdk.model.post_orders_request_payment"
...
function orders_api:get_orders(page, page_size, end_date, start_date, id, status)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/orders?endDate=%s&startDate=%s&page=%s&pageSize=%s&id=%s&status=%s",
			self.basePath, http_util.encodeURIComponent(end_date), http_util.encodeURIComponent(start_date), http_util.encodeURIComponent(page), http_util.encodeURIComponent(page_size), http_util.encodeURIComponent(id), http_util.encodeURIComponent(status));
	})
...
function orders_api:get_orders_by_id(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
...
function orders_api:post_orders(post_orders_request)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
	...
	if http_status:sub(1,1) == "2" then
		...
		return payconductor_sdk_post_orders_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function orders_api:post_orders_by_id_confirm(id, post_orders_request_payment)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/orders/%s/confirm",
			self.basePath, id);
	})
	...
	req:set_body(dkjson.encode(post_orders_request_payment))
	...
	if http_status:sub(1,1) == "2" then
		...
		return payconductor_sdk_post_orders_by_id_confirm_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function orders_api:post_orders_by_id_refund(id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
