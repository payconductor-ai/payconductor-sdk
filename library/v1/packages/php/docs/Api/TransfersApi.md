...
```php
getWithdraws($page, $page_size, $end_date, $start_date, $id)
```
...
try {
    $apiInstance->getWithdraws($page, $page_size, $end_date, $start_date, $id);
} catch (Exception $e) {
    echo 'Exception when calling TransfersApi->getWithdraws: ', $e->getMessage(), PHP_EOL;
}
```
...
try {
    ...
    echo 'Exception when calling TransfersApi->getWithdrawsById: ', $e->getMessage(), PHP_EOL;
}
```
...
try {
    ...
    echo 'Exception when calling TransfersApi->postWithdraws: ', $e->getMessage(), PHP_EOL;
}
```
