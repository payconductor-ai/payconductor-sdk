...
OrdersAPI_postOrdersByIdConfirm(apiClient_t *apiClient, char *id, post_orders_request_payment_t *post_orders_request_payment)
{
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_post_orders_request_payment = NULL;
    if (post_orders_request_payment != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_post_orders_request_payment = post_orders_request_payment_convertToJSON(post_orders_request_payment);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_post_orders_request_payment);
        localVarBodyLength = strlen(localVarBodyParameters);
    }
    list_addElement(localVarHeaderType,"application/json"); //produces
    list_addElement(localVarContentType,"application/json"); //consumes
    ...
    //}
    //nonprimitive not container
    post_orders_by_id_confirm_200_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *OrdersAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = post_orders_by_id_confirm_200_response_parseFromJSON(OrdersAPIlocalVarJSON);
        cJSON_Delete(OrdersAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
    ...
    free(localVarPath);
    free(localVarToReplace_id);
    if (localVarSingleItemJSON_post_orders_request_payment) {
        cJSON_Delete(localVarSingleItemJSON_post_orders_request_payment);
        localVarSingleItemJSON_post_orders_request_payment = NULL;
    }
    free(localVarBodyParameters);
    return elementToReturn;
end:
