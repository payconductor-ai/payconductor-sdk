...
DRAFTEXPIRATIONINSECONDS_ANY_OF_SCHEMAS = ["int"]

class DraftExpirationInSeconds(BaseModel):
    """
    ...
    # data type: int
    anyof_schema_2_validator: Optional[StrictInt] = Field(default=3600, description="Order expiration time in seconds")
    if TYPE_CHECKING:
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = DraftExpirationInSeconds.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in DraftExpirationInSeconds with anyOf schemas: int. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into DraftExpirationInSeconds with anyOf schemas: int. Details: " + ", ".join(error_messages))
        else:
            return instance
