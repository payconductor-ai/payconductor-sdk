...
# **get_orders**
> get_orders(page => $page, page_size => $page_size, end_date => $end_date, start_date => $start_date, id => $id, status => $status)
...
eval {
    $api_instance->get_orders(page => $page, page_size => $page_size, end_date => $end_date, start_date => $start_date, id => $id, status => $status);
};
if ($@) {
...
> PostOrdersByIdConfirm200Response post_orders_by_id_confirm(id => $id, post_orders_request_payment => $post_orders_request_payment)
...
my $id = "id_example"; # string | 
my $post_orders_request_payment = WWW::OpenAPIClient::Object::PostOrdersRequestPayment->new(); # PostOrdersRequestPayment | Payment data for the order (Pix, Credit Card, Bank Slip, NuPay, etc...)

eval {
    my $result = $api_instance->post_orders_by_id_confirm(id => $id, post_orders_request_payment => $post_orders_request_payment);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling OrdersApi->post_orders_by_id_confirm: $@\n";
}
```
