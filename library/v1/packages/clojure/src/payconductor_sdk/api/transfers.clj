(ns payconductor-sdk.api.transfers
...
(defn-spec get-withdraws-with-http-info any?
  ...
  ([page float?, pageSize float?, ] (get-withdraws-with-http-info page pageSize nil))
...
(defn-spec get-withdraws any?
  "List withdrawals
  Returns a paginated list of withdrawals and transfers. Filter by creation date or search by ID (withdrawal ID, external ID, acquirer ID, or End2EndID)"
  ([page float?, pageSize float?, ] (get-withdraws page pageSize nil))
  ([page float?, pageSize float?, optional-params any?]
   (let [res (:data (get-withdraws-with-http-info page pageSize optional-params))]
