...
(defn-spec get-orders-with-http-info any?
  ...
  ([page float?, pageSize float?, ] (get-orders-with-http-info page pageSize nil))
...
(defn-spec get-orders any?
  "List orders
  Retrieve a list of orders. Use date filter and pagination parameters to refine results as needed."
  ([page float?, pageSize float?, ] (get-orders page pageSize nil))
  ([page float?, pageSize float?, optional-params any?]
   (let [res (:data (get-orders-with-http-info page pageSize optional-params))]
...
(defn-spec post-orders-by-id-confirm-with-http-info any?
  "Confirm order
  Confirms a Draft order, setting the payment method and processing the charges associated with the order."
  [id string?, post-orders-request-payment post-orders-request-payment]
  (check-required-params id post-orders-request-payment)
  (call-api "/orders/{id}/confirm" :post
            {:path-params   {"id" id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    post-orders-request-payment
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["basicAuth"]}))

(defn-spec post-orders-by-id-confirm post-orders-by-id-confirm-200-response-spec
  "Confirm order
  Confirms a Draft order, setting the payment method and processing the charges associated with the order."
  [id string?, post-orders-request-payment post-orders-request-payment]
  (let [res (:data (post-orders-by-id-confirm-with-http-info id post-orders-request-payment))]
    (if (:decode-models *api-context*)
       (st/decode post-orders-by-id-confirm-200-response-spec res st/string-transformer)
       res)))
