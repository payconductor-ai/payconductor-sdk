  ...
  class NuPay < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::NuPay` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::NuPay`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @nu_pay.nil?
        invalid_properties.push('invalid value for "nu_pay", nu_pay cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] nu_pay Value to be assigned
    def nu_pay=(nu_pay)
      if nu_pay.nil?
        fail ArgumentError, 'nu_pay cannot be nil'
      end
