...
module OpenapiClient
  class CreditCard < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CreditCard`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @installments.nil?
        invalid_properties.push('invalid value for "installments", installments cannot be nil.')
      end

      if !@soft_descriptor.nil? && @soft_descriptor.to_s.length > 22
        invalid_properties.push('invalid value for "soft_descriptor", the character length must be smaller than or equal to 22.')
      end

      if !@soft_descriptor.nil? && @soft_descriptor.to_s.length < 1
        invalid_properties.push('invalid value for "soft_descriptor", the character length must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @installments.nil?
      return false if !@soft_descriptor.nil? && @soft_descriptor.to_s.length > 22
      return false if !@soft_descriptor.nil? && @soft_descriptor.to_s.length < 1
      true
    end
    ...
    def soft_descriptor=(soft_descriptor)
      if soft_descriptor.nil?
        fail ArgumentError, 'soft_descriptor cannot be nil'
      end

      if soft_descriptor.to_s.length > 22
        fail ArgumentError, 'invalid value for "soft_descriptor", the character length must be smaller than or equal to 22.'
      end

      if soft_descriptor.to_s.length < 1
        fail ArgumentError, 'invalid value for "soft_descriptor", the character length must be greater than or equal to 1.'
      end
    ...
    def hash
      [payment_method, card, installments, soft_descriptor].hash
    end
