...
module OpenapiClient
  class PostWithdraws200ResponsePayoutAccount < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PostWithdraws200ResponsePayoutAccount` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PostWithdraws200ResponsePayoutAccount`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @owner_document !~ pattern
        invalid_properties.push("invalid value for \"owner_document\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if @owner_document !~ Regexp.new(/^\d{11}$|^\d{14}$/)
    ...
    def owner_document=(owner_document)
      ...
      pattern = Regexp.new(/^\d{11}$|^\d{14}$/)
      if owner_document !~ pattern
        fail ArgumentError, "invalid value for \"owner_document\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [id, owner_document, owner_name, pix_key, pix_type].hash
    end
